/* =====================================================================
   ...
   Owners        | id | username | password | name | createdAt
   AdminActivityLog | id | adminId | adminName | action | sheetName | summary | createdAt
   SupportAgents | id | username | password | name | createdAt
   SupportConversations | id | userId | userName | userType | status | lastMessageAt | createdAt
   SupportMessages | id | conversationId | senderType | senderName | text | imageUrl | createdAt | read
...
async function sheetsonRequest(path, { method = "GET", query = {}, body } = {}) {
  ...
  try {
    ...
    // the browser never exposes which of the two, so name both possibilities.
    throw new Error("تعذر الوصول لقاعدة البيانات (تأكد من الإنترنت، ومن مشاركة الشيت مع google@sheetson.com، ومن عدم حظر الطلب بسبب CORS)");
  }
  if (!res.ok) {
    const detail = await res.text().catch(() => "");
    throw new Error(`Sheetson ${method} ${path} فشل: ${res.status} ${detail.slice(0, 200)}`);
  }
  return res.json();
}
...
const Sheet = {
  async list(name, { filter } = {}) {
    let all = [];
    let skip = 0;
    const limit = 100;      // Sheetson's max rows per request
    const maxRows = 1000;   // hard ceiling on how many rows we'll ever pull from one sheet
    ...
    all = all.slice(0, maxRows).filter((r) => r.id); // drop blank/placeholder rows with no id
...
document.getElementById("btn-login").addEventListener("click", async () => {
  ...
  try {
    ...
    enterApp();
  } catch (err) { showAuthError("تعذر الاتصال: " + (err.message || "خطأ غير معروف")); console.error(err); }
});
...
document.getElementById("btn-signup").addEventListener("click", async () => {
  ...
  try {
    ...
    enterApp();
  } catch (err) { showAuthError("تعذر إنشاء الحساب: " + (err.message || "خطأ غير معروف")); console.error(err); }
});
...
document.getElementById("btn-cc-lookup").addEventListener("click", async () => {
  ...
  } catch (err) { showAuthError("تعذر التحقق من الكود: " + (err.message || "خطأ غير معروف")); console.error(err); }
});
...
document.getElementById("btn-cc-complete").addEventListener("click", async () => {
  ...
  try {
    ...
    enterApp();
  } catch (err) { showAuthError("تعذر إكمال التسجيل: " + (err.message || "خطأ غير معروف")); console.error(err); }
});
...
document.getElementById("btn-forgot-submit").addEventListener("click", async () => {
  ...
  try {
    ...
    document.getElementById("forgot-form").hidden = true;
    document.getElementById("login-form").hidden = false;
  } catch (err) { showAuthError("تعذر تحديث كلمة المرور: " + (err.message || "خطأ غير معروف")); console.error(err); }
});
...
async function renderHome() {
  ...
  if (last) document.getElementById("continue-card").addEventListener("click", () => (last.taskType === "quiz" ? renderQuiz(last.taskObj) : renderTask(last.taskObj)));
...
async function openLeaderboard(grade) {
  ...
  try {
    ...
    }).filter((r) => r.count > 0).sort((a, b) => b.pct - a.pct).slice(0, 20);
...
async function renderLectureTasks(lectureId, lecture) {
  ...
  document.getElementById("back-course").addEventListener("click", () => history.back());
  document.getElementById("btn-vocab").addEventListener("click", () => renderVocab(lectureId));
  let noteTimer;
  ...
  try {
    ...
    wrap.querySelectorAll(".path-node").forEach((node) => {
      ...
      node.addEventListener("click", () => (t.type === "quiz" ? renderQuiz(t) : renderTask(t)));
    });
...
function renderTask(t) {
  ...
  el.innerHTML = `
    ...
    <h2>${t.title}</h2>${body}`;
  document.getElementById("back-task").addEventListener("click", () => { if ("speechSynthesis" in window) speechSynthesis.cancel(); history.back(); });
  ...
  if (video) {
    ...
    const savedPos = Number(localStorage.getItem(posKey));
    ...
    video.addEventListener("timeupdate", () => {
      const now = Date.now();
      if (now - saveThrottle > 4000) { saveThrottle = now; localStorage.setItem(posKey, String(video.currentTime)); }
    });
  } else completeTask();
}
...
async function renderQuiz(t) {
  ...
  function finish() {
    ...
    document.getElementById("quiz-done").addEventListener("click", () => history.back());
  }
  function draw() {
    ...
    document.getElementById("quiz-exit").addEventListener("click", () => {
      if (confirm("هل تريد الخروج؟ لن تُحتسب نتيجة هذا الاختبار")) { quizActive = false; if (countdownTimer) clearInterval(countdownTimer); history.back(); }
    });
...
async function renderVocab(lectureId) {
  ...
  function draw() {
    if (i >= words.length) {
      ...
      document.getElementById("vocab-done").addEventListener("click", () => history.back());
      return;
    }
    const w = words[i];
    el.innerHTML = `
      <div dir="rtl">
        ...
        <button class="speak-btn" id="speak">${svgIcon(ICONS.audio)}</button>
    ...
    document.getElementById("speak").addEventListener("click", () => speak(w.wordEn));
  ...
  try {
    ...
    wrap.innerHTML = faqs.map((f, i) => `
      <div class="path-card" style="margin-bottom:10px;cursor:pointer;" data-i="${i}">
        ...
        <div class="sub faq-answer" hidden style="margin-top:8px;line-height:1.7;">${f.answer}</div>
      </div>`).join("");
...
async function renderProfile() {
  ...
  document.getElementById("btn-change-pass").addEventListener("click", async () => {
    ...
    } catch (err) { toast("تعذر التحديث: " + (err.message || "")); console.error(err); }
  });
